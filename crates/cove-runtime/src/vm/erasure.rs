...
//! `cove-ir`'s own suite pins what each construct lowers to. This asks the
...
fn on_the_machine(source: &str) -> Answer {
    ...
    let program = match cove_ir::lower(&checked, &sources, &schemas()) {
        Ok(program) => program,
        Err(items) => panic!(
            "the program lowers:\n{}",
            items
    ...
    said(Vm::new(&runtime, &hosts, &program).run_entry("m", "main", Vec::new()))
}
...
///
/// `cove_ir::Inst::Trap` carries a static message and `Inst::Unbox` raises

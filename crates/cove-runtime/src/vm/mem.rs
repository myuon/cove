...
//!   cell it was written through — see [`crate::vm::cell`] — because
...
//! was taken: `benches/` on `--backend vm`, the minimum of ten interleaved
...
pub(crate) struct Space {
    ...
    ///
    /// A cell's lock is a word of the cell ([`crate::vm::cell`]), so waiting
...
impl Space {
    ...
    /// — it runs every [`crate::vm::exec::SAFEPOINT_STRIDE`] instructions and
...
///
/// The type a [`crate::vm::exec::Machine`] holds. Every address it reads or
...
mod tests {
    ...
    impl Table {
        ...
        fn structure(&mut self, name: &str, fields: &[(&str, LayoutId)]) -> LayoutId {
            let named: Vec<(Arc<str>, LayoutId)> = fields
                ...
                .collect();
            let (fields, words) = cove_ir::struct_layout(&named, &self.0);
            self.inline(
                name,
                Shape::Struct {
                    fields,
        ...
        fn enumeration(&mut self, name: &str, cases: &[(&str, Vec<LayoutId>)]) -> LayoutId {
            let named: Vec<(Arc<str>, Vec<LayoutId>)> = cases
                ...
                .collect();
            let (cases, payload) = cove_ir::enum_layout(&named, &self.0);
    ...
    // `Memory::string_bytes_into` and `Space::bytes_into`: a string longer
    // than a chunk read back whole, and appended rather than overwritten. All
    // three are gone with the last caller — `vm::intrinsics::operand`'s
    // `with_text`, whose own last caller was `String.indexOf` before ADR 0064
    // moved it into `std.string`. Nothing left reads a string's bytes a chunk
    // at a time: `Machine::string_bytes` reads one payload word at a time
    // through `Memory::payload`, and the boundary that reader crosses is
    // `a_run_crosses_a_chunk_boundary_whole`'s above.

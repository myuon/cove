...
fn expect_generated_source(
    ...
    match (&*result.case, result.payload.first()) {
        ("Ok", Some(Value::Str(source))) => Ok(source.to_string()),
        ("Err", payload) => Err(CliError::Message(
            payload.map(ToString::to_string).unwrap_or_default(),
        )),

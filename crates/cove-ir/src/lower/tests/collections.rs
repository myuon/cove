...
fn @m.xs() -> Array
  frame 6: s0:ref s1:int s2:int s3:int s4:ref s5:int
     ...
     4  int s5:int 0
     5  store-elem s4:ref s5:int s1:Int
     6  int s5:int 1
     7  store-elem s4:ref s5:int s2:Int
     8  int s5:int 2
     9  store-elem s4:ref s5:int s3:Int
    10  copy s0:Array s4:Array
    11  return s0:Array
"
    );
}
...
fn @m.xs() -> Array
  frame 6: s0:ref s1:int s2:int s3:int s4:int s5:ref
     ...
     5  int s1:int 0
     6  store-elem s5:ref s1:int s3..s4:m.Point
     7  copy s0:Array s5:Array
     8  return s0:Array
"
    );
}
...
fn @m.r() -> Range
  frame 8: s0:int s1:int s2:bool s3:int s4:int s5:int s6:int s7:bool
     0  int s3:int 0
     1  int s4:int 3
     2  copy s5:Int s3:Int
     3  copy s6:Int s4:Int
     4  bool s7:bool false
     5  copy s0..s2:Range s5..s7:Range
     6  return s0..s2:Range
"
    );
}
...
fn @m.total(Int) -> Int
  ...
  local n -> s0:Int [0, 20)
  local t -> s2:Int [1, 19)
  local i -> s3:Int [13, 14)
     0  int s2:int 0
     1  int s3:int 0
     2  copy s4:Int s3:Int
     3  copy s5:Int s0:Int
     4  bool s6:bool false
     5  copy s3:Int s4:Int
     6  copy s7:Int s5:Int
     7  copy s8:Bool s6:Bool
     8  int s9:int 1
     9  jump 12
    10  lt.int.branch s10:bool s3:int s7:int 18
    11  add.int s3:int s3:int s9:int
    12  lt.int.branch s10:bool s3:int s7:int 15
    13  add.int s2:int s2:int s3:int
    14  jump 10
    15  branch-false s8:bool 18
    16  eq.int.branch s10:bool s3:int s7:int 18
    17  jump 13
    18  copy s1:Int s2:Int
    19  return s1:Int
"
    );
}
...
fn @m.first(Array) -> Int
  ...
  local x -> s8:Int [9, 14)
     ...
     5  jump 7
     6  add.int s5:int s5:int s6:int
     7  lt.int.branch s7:bool s5:int s4:int 15
     8  load-elem s8:Int s3:ref s5:int
     9  eq.int.imm.branch s7:bool s8:int 0 11
    10  jump 6
    11  eq.int.imm.branch s7:bool s8:int 9 13
    12  jump 15
...
fn @m.v() -> Vector
  frame 6: s0:ref s1:int s2:int s3:ref s4:int s5:ref
     ...
     3  int s4:int 0
     4  store-elem s3:ref s4:int s1:Int
     5  int s4:int 1
     6  store-elem s3:ref s4:int s2:Int
...
fn @m.head(Vector) -> Option
  ...
  local v -> s0:Vector [0, 12)
     ...
     4  int s8:int 0
     5  ge.int.branch s9:bool s3:int s8:int 9

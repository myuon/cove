...
fn @m.f(Int) -> Result
  ...
  local n -> s0:Int [0, 11)
     0  gt.int.imm.branch s7:bool s0:int 0 4
     1  tag s4:tag Result.Ok
     2  copy s5:Unit s8:Unit
     3  jump 9
     4  str s9:ref \"assertion failed: `n > 0`\"
     5  assert.failed s9:ref
     6  copy s10:String s9:String
     7  tag s4:tag Result.Err
     8  copy s6:Error s10:Error
     9  copy s1..s3:Result s4..s6:Result
    10  return s1..s3:Result
"
    );
}

...
mod base {
    ...
    /// [`crate::Inst::RunSlice`], as the two opcodes its storage splits it
    /// into, beside the copy it is made of: the byte member is
    /// `String.sliceBytes`, and the word member `Array.slice`, `Vector.slice`
    /// and `Vector.toArray`, since ADR 0058 moved them into the standard
    /// library.
    pub const RUN_SLICE_BYTES: u8 = RUN_COPY_WORDS + 1;
    pub const RUN_SLICE_WORDS: u8 = RUN_SLICE_BYTES + 1;
    ...
    /// [`crate::Inst::FloatSqrt`], the last of ADR 0064's Decision 2 typed
    /// scalar operations — one opcode, for [`FLOAT_ABS`]' reason, and the one
...
pub enum Op {
    ...
    RunCopyWords,
    /// [`crate::Inst::RunSlice`] over [`crate::Storage::PackedBytes`]; the
    /// answer's layout is the row's `dst`, which is always `String`.
    RunSliceBytes,
    ...
    FloatRound,
    /// [`crate::Inst::FloatSqrt`]: one `Float` in, one `Float` out.
    FloatSqrt,
...
impl Op {
    ...
    pub fn fields(self) -> Fields {
        ...
        match self {
            ...
            // in, one `Float` out.
            Op::FloatRound | Op::FloatSqrt => fields(
                Operand::Word(FLOAT),
                Operand::Word(FLOAT),
                NONE,
                Payload::Empty,
            ...
            // The target layout rides in the payload because the IR carries it;
            // for bytes it is always `Program::str_layout`, which
            // `crate::verify` checks, and a word finish will name an `Array`.
            Op::RunFinishBytes => fields(
                Operand::Word(REF),
                Operand::Word(REF),
                NONE,
                one(Half::Layout),
...
mod tests {
    ...
    /// `AllocBuffer`, `AppendByte`, `AppendBytes` and `FinishBuffer` (now
    /// ADR 0058's `GrowableAllocBytes`, `GrowablePushByte`,
    /// `GrowableExtendBytes` and `RunFinishBytes`) brought
    ...
    /// questions — a hundred and eighty-two once issue #454's Step 2
    ...
    fn the_arithmetic_families_are_the_cross_products_the_adr_gives() {
        ...
        // And the fourth is one again, for the same reason, although this is
        // the one the machine really does have an instruction for: `sqrtsd`

...
//! element.
//!
//! Every Cove value is exactly one word. Nothing here occupies two slots —
//! not `dyn`, not a value the checker declined to type. Those are a `Ref` to
//! an object that carries its own description, which costs an allocation on
//! a path that was never going to be fast and buys a reference map that is
//! one bit per slot rather than a range table.

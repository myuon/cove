...
/// What the decision for `request` is, over the shipped samples, on `Vm`.
...
fn both_backends_answer_the_direct_invocation_the_same_way() {
    for pr in cove_rules::samples().values() {
        assert_eq!(
            evaluate(pr, &[], true).expect("Vm decides"),
...
/// The interpreter and `Vm` reach the same decisions over the embedded
/// entry.
///
